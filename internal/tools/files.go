...
func listFilesTool() *mcp.Tool {
	return &mcp.Tool{
		...
		Name:        "list_opportunity_files",
		Description: "List all files for a specific opportunity in Lever.",
...
func listFilesHandler(c client.LeverClient) mcp.ToolHandler {
	return func(ctx context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		data, err := c.Get(ctx, fmt.Sprintf("/opportunities/%s/files", id), params)
		if err != nil {
			return toolErrorf("Failed to list files: %v", err), nil
		}
